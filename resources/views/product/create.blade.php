                    ...
                    <div class="card-body">
                        ...
                        <form id="product-form">
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>{{trans('file.Arabic Name')}} *</strong> </label>
                                        <input type="text" name="name_ar" class="form-control" id="name_ar" aria-describedby="name_ar" required>
                                        <span class="validation-msg" id="name_ar-error"></span>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>{{trans('file.English Name')}} *</strong> </label>
                                        <input type="text" name="name_en" class="form-control" id="name_en" aria-describedby="name_en" required>
                                        <span class="validation-msg" id="name_en-error"></span>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                ...
                                <div id="combo" class="col-md-9 mb-1">
                                    ...
                                    <div class="search-box input-group mb-3">
                                        ...
                                        <input type="text" name="product_code_name" id="lims_productcodeSearch" placeholder="من فضلك استخدم الماسح او اكتب كود المنتح او الاسم...." class="form-control" />
                                    </div>
                                ...
                                <div class="col-md-4 category-form-group mainCategory">
                                    <div class="form-group">
                                        <label>{{trans('file.category')}} *</strong> </label>
                                        <div class="input-group">
                                          <select name="category_id" required class="selectpicker form-control" data-live-search="true" data-live-search-style="begins" title="اختر القسم">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>{{trans('file.App Price')}} *</strong> </label>
                                        <input type="number" name="app_price" required class="form-control" step="any">
                                        <span class="validation-msg"></span>
                                    </div>
                                </div>
...
<script type="text/javascript">
    @if (isset($_GET['p_id']))
       $('#product-form').append("<input type='hidden' name='redirect_to_new' value='{{$_GET['p_id']}}'>")
    ...
    function populate_category(unitID){
        ...
        $(document).on('change', "select[name='category_id']", function (e) {
            var parentCatDev = $(this).closest('.category-form-group');
            if (parentCatDev.hasClass('mainCatDiv')){
                $('.category-form-group').not('.mainCatDiv').remove();
            }
            if ($(document).find('.mainCatDiv').length == 0){
                parentCatDev.addClass('mainCatDiv');
            }
            if (parentCatDev.hasClass('clonedCategory')){
                parentCatDev.nextAll('.category-form-group').remove()
            }
            var catId = $("select[name='category_id']:last option:selected").val();
        
            $.ajax({
                        type:'GET',
                        url:'{{url("category/children")}}/'+catId,
                        success:function(response){
                            if (response.length == 0){
                                return;
                            }
                            var mainCat = $('div.category-form-group.mainCategory')
                            

                            var clonedCat = mainCat.clone()
                            clonedCat.find("select[name='category_id']").empty();
                            $.each(response, function(key, value) {
                                clonedCat.find("select[name='category_id']").append('<option value="'+ value.id +'">'+ value.name +'</option>');
                            });
                            clonedCat.removeClass('mainCategory');
                            clonedCat.removeClass('mainCatDiv');
                            clonedCat.addClass('clonedCategory');
                            clonedCat.insertAfter("div.category-form-group:last");
                            var clonedCat = $('.category-form-group.clonedCategory:last');
                            // $("select[name='category_id']:last").empty();

                            // $.each(data, function(key, value) {
                            //     $("select[name='category_id']:last").append('<option value="'+ key +'">'+ value +'</option>');
                            // });
                            clonedCat.find('select').selectpicker('refresh');
                            clonedCat.find('.bootstrap-select button:first').remove();
                            clonedCat.find('label').text('القسم الفرعي*');
                ...
                if ( $("#product-form").valid() && validate() ) {
                    ...
                    if(myDropzone.getAcceptedFiles().length) {
                        ...
                        $.ajax({
                            ...
                            success:function(response){
                                if (response === "1"){
                                    location.href = '../purchases/{{@$_GET["p_id"]}}/edit';    
                                }else{
