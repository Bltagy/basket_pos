...
class ReportController extends Controller
{
    ...
    public function productReportData(Request $request)
    {
        ...
        if ($request->input('search.value')) {
            ...
            $totalData = Product::whereTranslationLike('name', "%$search%")->count();
    ...
    public function userReport(Request $request)
    {
        ...
        $start_date = $data['start_date'] ? Carbon::parse($data['start_date'])->format('Y-m-d H:i:s') : date('Y-m-d') . ' 00:00:00';
        $end_date = $data['end_date'] ? Carbon::parse($data['end_date'])->format('Y-m-d H:i:s') : date('Y-m-d') . ' 23:59:00';
//        dd(Carbon::now()->format('Y-m-d H:i:s'));
//        dd(Carbon::parse($data['start_date'])->format('Y-m-d H:i:s'));
//        dd([$data['start_date'],$start_date]);
        $lims_product_sale_data = [];
        $lims_product_purchase_data = [];
        $lims_product_quotation_data = [];
        $lims_product_transfer_data = [];
//        $role = Role::find(User::find($user_id)->role_id);
//        if ($role->hasPermissionTo('sales-app-add')){
//            $lims_sale_data = Sale::with('customer', 'warehouse')->whereNotNull('order_id')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->orderBy('created_at', 'desc')->get();
//        }else{
//        }

        $lims_sale_data = Sale::with('customer', 'warehouse')
            ->where('user_id', $user_id)
//            ->take(10)
            ->whereBetween('created_at', [$start_date, $end_date])
            ->orderBy('created_at', 'desc');
        $lims_sale_ids = $lims_sale_data->pluck('id')->toArray();
        $lims_sale_data = $lims_sale_data->get();
//        dd($lims_sale_data);
