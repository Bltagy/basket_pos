...
class SaleController extends Controller
{
    ...
    public function cashierSales(Request $request, $id)
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('sales-index')) {
            $permissions = Role::findByName($role->name)->permissions;
            ...
            return view('sale.cashier-index',compact('starting_date','cashier', 'ending_date', 'warehouse_id', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_reward_point_setting_data', 'lims_account_list', 'lims_warehouse_list', 'all_permission'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function saleData(Request $request)
    {
        ...
        elseif($warehouse_id != 0)
            $totalData = Sale::where('warehouse_id', $warehouse_id)->whereNull('order_id')->whereDate('created_at', '>=' ,$request->input('starting_date'))->whereDate('created_at', '<=' ,$request->input('ending_date'))->count();
        elseif($request->input('cashier_id'))
            $totalData = Sale::where('user_id',$request->input('cashier_id'))->whereDate('created_at', '>=' ,$request->input('starting_date'))->whereNull('order_id')->whereDate('created_at', '<=' ,$request->input('ending_date'))->count();
        else
            $totalData = Sale::whereDate('created_at', '>=' ,$request->input('starting_date'))->whereNull('order_id')->whereDate('created_at', '<=' ,$request->input('ending_date'))->count();
        ...
        if(empty($request->input('search.value'))) {
            ...
            elseif($request->input('cashier_id'))
                $sales = Sale::with('biller', 'customer', 'warehouse', 'user')
                        ->whereNull('order_id')
                        ->where('user_id',$request->input('cashier_id'))
                        ->whereDate('created_at', '>=' ,$request->input('starting_date'))
                        ->whereDate('created_at', '<=' ,$request->input('ending_date'))
                        ->offset($start)
                        ->limit($limit)
        ...
        else
        {
            ...
            elseif($request->input('cashier_id')){
            $sales =  Sale::select('sales.*')
                        ->where('sales.user_id', $request->input('cashier_id'))
                        ->whereNull('order_id')
                        ->with('biller', 'customer', 'warehouse', 'user')
                        ->join('customers', 'sales.customer_id', '=', 'customers.id')
                        ->join('billers', 'sales.biller_id', '=', 'billers.id')
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['customer'] = $sale->customer ? $sale->customer->name.'<input type="hidden" class="deposit" value="'.($sale->customer->deposit - $sale->customer->expense).'" />'.'<input type="hidden" class="points" value="'.$sale->customer->points.'" />':"";
                ...
                $nestedData['sale'] = array( '[ "'.date(config('date_format'), strtotime($sale->created_at->toDateString())).'"', ' "'.$sale->reference_no.'"', ' "'.$sale_status.'"', ' "'.$sale->biller->name.'"', ' "'.$sale->biller->company_name.'"', ' "'.$sale->biller->email.'"', ' "'.$sale->biller->phone_number.'"', ' "'.$sale->biller->address.'"', ' "'.$sale->biller->city.'"', ' "'.

                // $sale->customer->name.'"', ' "'.$sale->customer->phone_number.'"', ' "'.$sale->customer->address.'"', ' "'.$sale->customer->city.
                '"', ' "'.$sale->id.'"', ' "'.$sale->total_tax.'"', ' "'.$sale->total_discount.'"', ' "'.$sale->total_price.'"', ' "'.$sale->order_tax.'"', ' "'.$sale->order_tax_rate.'"', ' "'.$sale->order_discount.'"', ' "'.$sale->shipping_cost.'"', ' "'.$sale->grand_total.'"', ' "'.$sale->paid_amount.'"', ' "'.preg_replace('/[\n\r]/', "<br>", $sale->sale_note).'"', ' "'.preg_replace('/[\n\r]/', "<br>", $sale->staff_note).'"', ' "'.$sale->user->name.'"', ' "'.$sale->user->email.'"', ' "'.$sale->warehouse->name.'"', ' "'.$coupon_code.'"', ' "'.$sale->coupon_discount.'"]'
                );
                $data[] = $nestedData;
            }
        }
    ...
    public function store(Request $request)
    {
        ...
        $data['created_at'] = isset($data['created_at']) ? Carbon::parse($data['created_at'])->toDateTimeString() : Carbon::now()->toDateTimeString();
        ...
        $mail_data['email'] = 'example@exmpale.com';
...
//            ->where('qty','>',0)
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                ...
                $amount_got = CashierLog::where('user_id',Auth::id())
                                        ->where('date',$mytime->toDateString())
                                        ->whereNull('time_closed')
                                        ->first();
                ...
                if ($amount_got){
                    $sales_amount =$sales_amount->where('cashier_log_id',$amount_got->id);
                }
                $sales_amount =$sales_amount->sum('grand_total');
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $product_image = explode(",", $lims_product_data->image);
        $product_image = htmlspecialchars($product_image[0]);
        $product[] = url('public/images/product', $product_image);
