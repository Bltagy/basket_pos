...
class AppSaleController extends Controller
{
    ...
    {
        $delivery_id = $request->has('delivery_id')?$request->delivery_id:0;
        $role = Role::find(Auth::user()->role_id);
        if ($role->hasPermissionTo('sales-app-add')) {
            ...
            $deliveryMen = User::where('role_id', 7)->get();

            return view('appSale.index', compact('delivery_id','starting_date', 'deliveryMen', 'ending_date', 'warehouse_id', 'lims_gift_card_list', 'lims_pos_setting_data', 'lims_reward_point_setting_data', 'lims_account_list', 'lims_warehouse_list', 'all_permission'));
        } else
    ...
    public function saleData(Request $request)
    {
        ...
        elseif ($warehouse_id != 0)
            $totalData = Sale::where('warehouse_id', $warehouse_id)->whereNotNull('order_id')->whereDate('created_at', '>=', $request->input('starting_date'))->whereDate('created_at', '<=', $request->input('ending_date'))->count();
        else
            $totalData = Sale::whereDate('created_at', '>=', $request->input('starting_date'))->whereNotNull('order_id')->whereDate('created_at', '<=', $request->input('ending_date'))->count();
        ...
        if (!empty($sales)) {
            foreach ($sales as $key => $sale) {
                ...
                if ($sale->sale_status == 1) {
                    ...
                    $nestedData['sale_status'] = '<div class="badge badge-warning">مع الطيار</div>';
                    $sale_status = 'مع الطيار';
                } else {
                ...
                $deliveryMen = User::where('role_id', 7)->get();
                $disabled = $sale->delivery_id ?"":"";
                $nestedData['paid_amount'] = '<select required name="delivery_id" '.$disabled.' data-sale-id="' . $sale->id . '" class="selectpicker form-control" data-live-search="true" id="delivery_id" data-live-search-style="begins" title="اختر الطيار">';
                foreach ($deliveryMen as $customer) {
                    $selected =  ($sale->delivery_id == $customer->id) ? 'selected' : '';
                    $nestedData['paid_amount'] .= '<option ' . $selected . ' value="' . $customer->id . '"> ' . $customer->name . '("' . $customer->phone_number . ' ")</option>';
                }
                $nestedData['paid_amount'] .= '</select>';
                $nestedData['due'] = $sale->delivery_at ? date('h:s A', strtotime($sale->delivery_at)) : "00:00";
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                <li><a href="' . route('appSale.invoice', $sale->id) . '" class="btn btn-link"><i class="fa fa-copy"></i> ' . trans('file.Generate Invoice') . '</a></li>
                                <li>
                ...
                if (in_array("sales-edit", $request['all_permission'])) {
                    ...
                    if ($sale->sale_status != 3)
                        $nestedData['options'] .= '<li>
                            <a href="' . route('appSales.edit', $sale->id) . '" class="btn btn-link"><i class="dripicons-document-edit"></i> ' . trans('file.edit') . '</a>
                            </li>';
                    else
                        $nestedData['options'] .= '<li>
                        <a href="' . route('appSales.edit', $sale->id) . '" class="btn btn-link"><i class="dripicons-document-edit"></i> ' . trans('file.edit') . '</a>
                    ...
                    if ($sale->sale_status != 1)
                        $nestedData['options'] .= '<li>
                            <a href="' . route('appSales.edit', $sale->id) . '?complete=1" class="btn btn-link"><i class="dripicons-document-edit"></i> اكتمل</a>
                            </li>';
                }
                // $nestedData['options'] .= 
                //     '<li>
                ...
                //     <li>
                //         <button type="button" class="complete-sale btn btn-link" data-id = "'.$sale->id.'" ><i class="fa fa-plus"></i> اكتمل</button>
                //     </li>
                //     <li>
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            return view('appSale.create', compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_reward_point_setting_data'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        // $data['warehouse_id'] = $data['warehouse_id'] ?$data['warehouse_id'] :0;
    ...
    {
        // $lims_product_warehouse_data = Product::whereHas('warehouse', function ($query) use($id) {
        ...
        foreach ($lims_product_warehouse_data as $product_warehouse) {
            ...
            $lims_product_data = Product::find($product_warehouse->id);
    ...
    public function posSale()
    {
        ...
        if ($role->hasPermissionTo('sales-app-add')) {
            ...
            $lims_reward_point_setting_data = RewardPointSetting::latest()->first();
            $lims_tax_list = Tax::where('is_active', true)->get();
            $lims_product_list = Product::select('id', 'name', 'code', 'image')->ActiveFeatured()->get();
    ...
    public function getProductByFilter($category_id, $brand_id)
    {
        ...
        foreach ($lims_product_list as $product) {
            if ($product->is_variant) {
                ...
                $productModel = Product::select('id')->find($product->id);
                $data['name'][$index] = $productModel->translate('ar') ? $productModel->translate('ar')->name : '';
    ...
    public function saleByCsv()
    {
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            return view('appSale.import', compact('lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function createSale($id)
    {
        ...
        if ($role->hasPermissionTo('sales-edit')) {
            ...
            return view('appSale.create_sale', compact('lims_biller_list', 'lims_customer_list', 'lims_warehouse_list', 'lims_tax_list', 'lims_sale_data', 'lims_product_sale_data', 'lims_pos_setting_data', 'lims_brand_list', 'lims_category_list', 'lims_coupon_list', 'lims_product_list', 'product_number', 'lims_customer_group_all'));
        } else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function edit($id)
    {
        ...
        $role = Role::find(Auth::user()->role_id);
        if ($role->hasPermissionTo('sales-edit')|| request()->has('complete')) {
            $lims_customer_list = Customer::where('is_active', true)->get();
            $lims_warehouse_list = Warehouse::where('is_active', true)->get();
            $lims_biller_list = Biller::where('is_active', true)->get();
            $lims_tax_list = Tax::where('is_active', true)->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        foreach ($lims_product_sale_data as  $key => $product_sale_data) {
            ...
            // $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($lims_product_data->id, $data['warehouse_id'])->first();
            // $quantity = $qty[$i] * $product_sale_data->operation_value;
            // //deduct quantity from warehouse
            // if ($lims_product_warehouse_data){
            //     $quantity = $quantity ? $quantity :0;
            //     $lims_product_warehouse_data->qty -= $quantity;
            ...
            if (($lims_sale_data->sale_status == 1 || $lims_sale_data->sale_status == 3) && ($lims_product_data->type == 'combo')) {
                $product_list = explode(",", $lims_product_data->product_list);
                $variant_list = explode(",", $lims_product_data->variant_list);
                if ($lims_product_data->variant_list)
                    $variant_list = explode(",", $lims_product_data->variant_list);
            ...
            } elseif (($lims_sale_data->sale_status == 1 || $lims_sale_data->sale_status == 3) && ($product_sale_data->sale_unit_id != 0)) {
        ...
        foreach ($product_id as $key => $pro_id) {
            ...
            if ($sale_unit[$key] != 'n/a') {
                ...
                $sale_unit_id = $lims_sale_unit_data->id;
                if ($data['sale_status'] == 1 || $data['sale_status'] == 3) {
        ...
        $mytime = Carbon::now();
        $cashierLog = CashierLog::where('user_id',Auth::id())
                                        ->where('date',$mytime->toDateString())
                                        ->whereNull('time_closed')
                                        ->first();
        $data['cashier_log_id'] = $cashierLog ? $cashierLog->id :0;
        ...
        return redirect('appSales')->with('message', $message);
    }

    public function printLastReciept()
    {
        $sale = Sale::where('sale_status', 1)->latest()->first();
        return redirect()->route('appSale.invoice', $sale->id);
    }
    ...
    public function genInvoice($id)
    {
        ...
        return view('appSale.invoice', compact('lims_sale_data', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
    }
    ...
    public function setSaleDelivery(Request $request)
    {
        $data = $request->all();
        $sale = Sale::find($data['saleId']);
        $hasDeliveryId = $sale->delivery_id ? true:false;
        $sale->delivery_id = $data['deliveryId'];
        $sale->sale_status = 3;
        $sale->delivery_at = now();
        $mytime = Carbon::now();
        $cashierLog = CashierLog::where('user_id',Auth::id())
                                        ->where('date',$mytime->toDateString())
                                        ->whereNull('time_closed')
                                        ->first();
        $sale->cashier_log_id = $cashierLog ? $cashierLog->id :0;
        $sale->save();
        if($hasDeliveryId){
            return 'success';
        }
        $id = $data['saleId'];
        $lims_sale_data = $sale;
        $product_id = Product_Sale::where('Sale_id', $id)->pluck('product_id')->toArray();
        $qty = Product_Sale::where('Sale_id', $id)->pluck('qty')->toArray();
        $product_batch_id = Product_Sale::where('sale_id', $id)->pluck('product_batch_id')->toArray();
        foreach ($product_id as $key => $pro_id) {
            $lims_product_data = Product::find($pro_id);
            ...
            if ($lims_product_data->is_variant) {
                ...
                $lims_product_warehouse_data = Product_Warehouse::FindProductWithoutVariant($pro_id, 1)
                    ->first();
            }
            $lims_product_data->qty -= $new_product_qty;
            $lims_product_data->save();
