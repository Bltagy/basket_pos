...
class CategoryController extends Controller
{
    public function index()
    {
        ...
        if($role->hasPermissionTo('category')) {
            $lims_categories_ob = Category::where('is_active', true)->get();
            $lims_categories = [];
            foreach ($lims_categories_ob as $key => $category) {
                $lims_categories[$category->id] = $category->translate('ar') ? $category->translate('ar')->name : '';
            }
    ...
    public function categoryData(Request $request)
    {
        ...
        if(!empty($categories))
        {
            foreach ($categories as $key=>$category)
            {
                ...
                $nestedData['name'] = $category->translate('ar') ? $category->translate('ar')->name : '';
    ...
    public function store(Request $request)
    {
        ...
        $lims_category_data['en']= ['name' => $request->name_en];
        $lims_category_data['ar']= ['name' => $request->name_ar];
            
        $lims_category_data['parent_id'] = $request->parent_id;
        $lims_category_data['is_active'] = true;
        $cat = Category::create($lims_category_data);
        $send_to_app = file_get_contents('https://app2.basketstore.net/api/posOperation/category/create/'.$cat->id);
    ...
    public function edit($id)
    {
        ...
        $lims_category_data['name_ar'] = $lims_category_data->translate('ar') ? $lims_category_data->translate('ar')->name : '';
        $lims_category_data['name_en'] = $lims_category_data->translate('en') ? $lims_category_data->translate('en')->name : '';
        if($lims_parent_data)
    ...
    public function update(Request $request)
    {
        ...
        $input['en']= ['name' => $request->name_en];
        $input['ar']= ['name' => $request->name_ar];
        $input['synced']= 0;
        $lims_category_data = Category::findOrFail($request->category_id);
        $lims_category_data->update($input);
        $send_to_app = file_get_contents('https://app2.basketstore.net/api/posOperation/category/update/'.$lims_category_data->id);
    ...
    public function deleteBySelection(Request $request)
    {
        ...
        foreach ($category_id as $id) {
            ...
            $send_to_app = file_get_contents('https://app2.basketstore.net/api/posOperation/category/delete/'.$id);
        }
    ...
    public function destroy($id)
    {
        ...
        if($lims_category_data->image)
            unlink('public/images/category/'.$lims_category_data->image);
        $lims_category_data->save();
        $send_to_app = file_get_contents('https://app2.basketstore.net/api/posOperation/category/delete/'.$lims_category_data->id);
        return redirect('category')->with('not_permitted', 'Category deleted successfully');
    }
    public function getChildren($id)
    {
        $lims_category_data = Category::findOrFail($id);
        return $lims_category_data->children;
    }
}
